...
void plotresponse_L3(TString inputFile = "",
  ...
  string outfolder = outputDir.IsNull()
                         ? Form("L3plots_%s_%s", mode.Data(), tag.Data())
                         : Form("%s/%s_%s", outputDir.Data(), mode.Data(), tag.Data());
  ...
  if (balance_dist) {
    ...
    for (int iPt = 1; iPt <= nPtBins; iPt++) {
      ...
      for (int iAlpha = 1; iAlpha <= nAlphaBins; iAlpha++) {
        ...
        h_bal->GetXaxis()->SetTitle(Form("B_{%s+jet}", refName.Data()));
        ...
        if (balance_dist_mc) {
          ...
          h_bal_norm->GetXaxis()->SetTitle(Form("B_{%s+jet}", refName.Data()));
          h_bal_norm->GetYaxis()->SetTitle(Form("1/N dN/dB_{%s+jet}", refName.Data()));
