...
// Shows raw distributions (Data or MC) with optional normalized MC vs Data
// comparison
//
...
//   root -l -b -q 'plotresponse_L3.C("data.root", "Data", false, "mc.root")' //
//   with MC comparison
...
static void NormalizeToUnityWidth(TH1 *h) {
  ...
  if (integralWidth <= 0)
    return; // assume non-zero for normal inputs
...
static void DrawSelectionText(double ptMin, double ptMax, double alphaCutMax,
  ...
  latex.DrawLatex(0.18, 0.86,
                  Form("%.0f < p_{T}^{#gamma} < %.0f GeV", ptMin, ptMax));
  latex.DrawLatex(0.18, 0.81, Form("#alpha < %.2f", alphaCutMax));
  latex.DrawLatex(0.18, 0.76, "B_{#gamma+jet} = p_{T}^{jet}/p_{T}^{#gamma}");
  if (hasMC) {
    latex.DrawLatex(0.18, 0.71,
                    Form("#LTB_{#gamma+jet}^{Data}#GT = %.3f", meanData));
    latex.DrawLatex(0.18, 0.66,
                    Form("#LTB_{#gamma+jet}^{MC}#GT = %.3f", meanMC));
  } else {
...
void plotresponse_L3(TString inputFile = "", TString tag = "Data",
                     bool isMC = false, TString mcFile = "",
  ...
  if (inputFile.IsNull()) {
    ...
    cout << "       plotresponse_L3.C(\"data.root\", \"Data\", false, "
            "\"mc.root\")  // for comparison"
         << endl;
    return;
  }
  ...
  cout << "Tag: " << tag << endl;
  if (mcFilePtr)
    cout << "MC comparison file: " << mcFile << endl;
  cout << "============================================" << endl;

  // Create output directory
  string outfolder = outputDir.IsNull()
                         ? Form("L3plots_%s", tag.Data())
                         : Form("%s/%s", outputDir.Data(), tag.Data());
  ...
  if (mcFilePtr) {
    ...
    if (!mcHistDir) {
      cout << "WARNING: Cannot find MC histogram directory: " << histoPath
           << endl;
      mcFilePtr = nullptr;
    }
  }
  ...
  for (const auto &config : histConfigs) {
    ...
    if (!mcFilePtr) {
      ...
      CMS_lumi(c, 0, 0);
      c->Print(Form("%s/%s_%s.png", outfolder.c_str(), histName.c_str(),
                    tag.Data()));
    ...
    if (hMC) {
      ...
      TH1D *hMC_norm =
          (TH1D *)hMC->Clone(Form("hMC_norm_%s", histName.c_str()));
      ...
      CMS_lumi(c, 0, 0);
      c->Print(
          Form("%s/%s_comparison.png", outfolder.c_str(), histName.c_str()));
  ...
  if (balance_dist) {
    ...
    cout << "Balance distribution entries: " << balance_dist->GetEntries()
         << endl;
    ...
    for (int iPt = 1; iPt <= nPtBins; iPt++) {
      ...
      for (int iAlpha = 1; iAlpha <= nAlphaBins; iAlpha++) {
        const double alphaCutMax =
            balance_dist->GetYaxis()->GetBinUpEdge(iAlpha);
        const double alphaEps = 1e-6;
        const int alphaMaxBin =
            balance_dist->GetYaxis()->FindBin(alphaCutMax - alphaEps);
        ...
        h_bal->SetName(
            Form("balance_pt%.0f_%.0f_alphalt%.3f", ptMin, ptMax, alphaCutMax));
        ...
        if (!mcFilePtr) {
          ...
          c->Print(Form("%s/balance_pt%.0f-%.0f_alphalt%.3f_%s.png",
                        outfolder.c_str(), ptMin, ptMax, alphaCutMax,
                        tag.Data()));
        ...
        if (balance_dist_mc) {
          ...
          h_bal_mc->SetName(Form("balance_mc_pt%.0f_%.0f_alphalt%.3f", ptMin,
                                 ptMax, alphaCutMax));
          ...
          TH1D *h_bal_norm =
              (TH1D *)h_bal->Clone(Form("balance_norm_pt%.0f_%.0f_alphalt%.3f",
                                        ptMin, ptMax, alphaCutMax));
          TH1D *h_bal_mc_norm = (TH1D *)h_bal_mc->Clone(
              Form("balance_mc_norm_pt%.0f_%.0f_alphalt%.3f", ptMin, ptMax,
                   alphaCutMax));

          NormalizeToUnityWidth(h_bal_norm);
          NormalizeToUnityWidth(h_bal_mc_norm);
          const double maxYBal =
              TMath::Max(h_bal_norm->GetMaximum(), h_bal_mc_norm->GetMaximum());
    ...
    cout << "Created " << nBalancePlots << " balance distribution plots"
         << endl;
