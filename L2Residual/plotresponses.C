...
namespace {
int edgeToInt(double x) { return static_cast<int>(std::lround(x)); }
} // namespace

void plotresponses(string input = "L2residuals_pbpbreco_rereco_zb_jetid.root",
                   string plottag = "ZB", bool closure = false) {
  ...
  float etalimit[] = {3.0, 3.0, 2.964 - 0.08, 2.5,
                      1.93 + 0.12}; // for shady box, do manually before
                                    // figuring out something smart
  bool drawbox = false;
  ...
  for (int i = 0; i < static_cast<int>(histograms::nptforjec); ++i) {
    ...
    string namelabel =
        Form("pbpb_%dto%dalpha%.1f_%s", pt1, pt2, alpha, plottag.c_str());

    auto mc = (TH1D *)file->Get(Form("mc_pt%dto%d_alpha%.1f", pt1, pt2, alpha));
    auto data =
        (TH1D *)file->Get(Form("dt_pt%dto%d_alpha%.1f", pt1, pt2, alpha));
    ...
    float shadeEdge = 3.0;
    if (i < static_cast<int>(sizeof(etalimit) / sizeof(etalimit[0])))
      shadeEdge = etalimit[i];
    ...
    c1->Print(
        Form("response_eta_%.1f_%.1f_%s.pdf", eta1, eta2, namelabel.c_str()));
  }
}
