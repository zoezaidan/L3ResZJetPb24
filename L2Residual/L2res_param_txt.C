...
void L2res_param_txt(TString inFileName = "ptfits/testing_pt_dep.root", TString kfactFile = "L2fits_AK4CHSjetveto_all/correctionfile.root", TString outFileName = "L2residual-param.txt", bool loglin = true, bool kfsr = false) {
   ...
   for (int etabin = kfacts->GetXaxis()->GetNbins(); etabin >= 1; --etabin) {
     ...
     if (kfacts->GetXaxis()->GetBinLowEdge(etabin) < 2.5) {
       ...
       else txtfile << -loweta << " " << higheta << " "  << nparams<< " "  << " " <<  ptmin << " " << ptmax << " " << params->GetParameter(0) << " " << (loglin ? params->GetParameter(1) : 0) <<  " " << (kfsr ? kfacts->GetBinContent(etabin) :0 ) << std::endl;
  ...
  for (int etabin = 1; etabin <= kfacts->GetXaxis()->GetNbins(); ++etabin) {
    ...
    if (kfacts->GetXaxis()->GetBinLowEdge(etabin) < 2.5) {
      txtfile << loweta << " " << higheta << " "  << nparams<< " "  << " " <<  ptmin << " " << ptmax << " " << params->GetParameter(0) << " " << (loglin ? params->GetParameter(1) : 0) <<  " " << (kfsr ? kfacts->GetBinContent(etabin) : 0)  << std::endl;
    }
  }
